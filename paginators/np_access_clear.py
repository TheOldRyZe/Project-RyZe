...
class noPrefixAccessClearButtons(discord.ui.View):
    ...
    async def yes(self, interaction: discord.Interaction, button: discord.ui.Button):
        if await self.interaction_check(interaction):
            await clear_noprefix_access_users()
            await interaction.response.edit_message(
                embed=discord.Embed(
                    description="Cleared All No-Prefix Access Users From The List",
                    color=config.color,
                ),
                view=None,
            )
            embed1=discord.Embed(description=f"**No-Prefix Access Cleared:**", color=config.color)
