...
class NoprefixButtons(discord.ui.View):
    ...
    async def show_current_page(self, interaction):
        ...
        embed = discord.Embed(
            title=f"No-Prefix Users - Page {self.current_page + 1}/{len(self.pages)}",
            color=config.color,
        )
    ...
    @discord.ui.button(label=emojis.first, style=discord.ButtonStyle.grey)
    ...
    @discord.ui.button(label=emojis.last, style=discord.ButtonStyle.grey)
    async def last_page(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if await self.interaction_check(interaction):
