...
class blacklistClearButtons(discord.ui.View):
    def __init__(self, ctx, client, type, message=None):
    ...
    async def yes(self, interaction: discord.Interaction, button: discord.ui.Button):
        if await self.interaction_check(interaction):
            await clear_blacklist(self.type)
            await interaction.response.edit_message(
                embed=discord.Embed(
                    description=f"Cleared All Blacklisted {self.type.capitalize()} From The List",
                    color=config.color,
                ),
                view=None,
            )
            embed1=discord.Embed(description=f"**Blacklist Cleared:**", color=config.color)
