...
class WhitelistClear(discord.ui.View):
    ...
    @discord.ui.button(emoji=emojis.tick, style=discord.ButtonStyle.grey)
    async def yes(self, interaction: discord.Interaction, button: discord.ui.Button):
        if await self.interaction_check(interaction):
            await clear_whitelisted_users(interaction.guild.id)
            await interaction.response.edit_message(
                embed=discord.Embed(
                    description="Cleared All Whitelisted Users From The List",
                    color=config.color,
                ),
                view=None,
            )
            embed1=discord.Embed(description=f"**Whitelisted Users Cleared:**", color=config.color)
