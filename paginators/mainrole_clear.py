...
class MainroleClear(discord.ui.View):
    ...
    async def yes(self, interaction: discord.Interaction, button: discord.ui.Button):
        if await self.interaction_check(interaction):
            await clear_mainroles(interaction.guild.id)
            await interaction.response.edit_message(
                embed=discord.Embed(
                    description="Cleared All Mainroles From The List",
                    color=config.color,
                ),
                view=None,
            )
            embed1=discord.Embed(description=f"**Mainroles Cleared:**", color=config.color)
