...
class AntiBot(commands.Cog):
    ...
    async def on_ready(self):
        print("Antinuke : Event AntiBot is Ready")
    ...
    async def on_member_join(self, member) -> None:
        ...
        async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
            ...
            if difference > 3600:
                return
            check = await antinuke_event_check(executor, guild, "Bot_Add", self.client)
            if check=="Antinuke_Disabled":
                return
            elif check=="Is_Guild_Owner":
                return
            elif check=="Not_Whitelisted":
                await ban(guild.id, executor.id, reason="Bot Add | Not Whitelisted")
                try:
                    await ban(guild.id, entry.target.id, reason="Illegal Bot | Not Whitelisted")
