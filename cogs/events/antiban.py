...
class AntiBan(commands.Cog):
    ...
    async def on_ready(self):
        print("Antinuke : Event AntiBan is Ready")
    ...
    async def on_member_ban(self, guild, user) -> None:
        ...
        async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
            ...
            if check=="Antinuke_Disabled":
                ...
                await ban(guild.id, executor.id, reason="Member Ban | Not Whitelisted")
                try:
                    await guild.unban(user, reason='RyZe Recovery | Anti Ban')
